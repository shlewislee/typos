...
| `POST` | `/print/template`    | Render a named template (multipart)                           |
...
| Field           | Description                                              |
| --------------- | -------------------------------------------------------- |
| `rotate_image`  | `"true"`/`"1"` to rotate, `"false"`/`"0"` not to rotate   |
| `dither_method` | `0` (Atkinson), `1` (FloydSteinberg), `2` (StevenPigeon) |
| `gamma`         | Float (e.g. `"4.5"`)                                     |

Unset fields fall back to the server defaults (`--gamma`, `--dither-method`).
...
**Errors:** `400`, `404`, `415`, `503` — all return `{"message": "..."}`.
**Wrong content type:** `415 {"message": "use multipart/form-data"}` (all print endpoints require multipart).
...
- All print endpoints require `multipart/form-data`; JSON bodies are rejected with `415`.
